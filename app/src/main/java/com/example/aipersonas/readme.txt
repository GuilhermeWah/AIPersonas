...
#Architecture Database (Firestore) : Updated Nov 05;
Firestore Root
└── Users (Collection)
    └── UserID (Document)
        ├── Personas (Sub-Collection)
...
We set up our collections and subcollections on Firestore. We also found that the userId generated by the Firebase Authentication
was not being saved on Firestore automatically. The reason for that was mainly because the function handleSignUp() was not
invoking FirebaseFirestore.getInstance().collection()...
The collections and subcollections are now working, whenever account is created the userId from Authentication
is stored on Firestore, as well as for the chats and messages.



05/11/2024 : Starting the testing phase
- In our previous version the chatList (ChatAdapter) was not explicitly initialized, which could lead to null pointer exceptions if the list was accessed before being assigned.
- Now our chatList is initialized to a new ArrayList<>();. This ensures that the adapter always has a non-null list to work with,
- Clearing the chatList and Adding New Data;  instead of just assigning the new list, we are now clearing the existing list and adding
all the new items to it; this way we prevent issues as the reference changes since we are dealing with updates from both Room and Firestore
- We implemented  deleteAllChatsForUser(String userId) on ChatDAO. This will clear all chats associated with a user whenever needed
, which is essential for syncing with Firestore.
- Firebase doesnt support autogeneration of IDS like Room does.  Because of that, instead of using
@PrimaryKey(autogenerate=true), we gonna use the same idea we used on the Persona modal. We are going to
use UUID, this way we can generate a unique identifier. Eg: this.chatId = UUID.randomUUID().toString();
- We were facing a bug with ChatID when storing in Firestore, the reason is because it was set as Integer.
We need to use String. For this reason I changed the chatId to String on chatmodel.

### big bug:  spent one day trying to figure out why was getting the following on LogCat:
...
It was right after adding a new functionality ChatListActivity and changing the Firebase firestore
structure.  I was getting this because old users didn't have the new collections.

@We are not working on this right now, but whenever we update our Firestore structure, this bug will
arise. We have to think about a way to handle this and make sure the old users are updated.
